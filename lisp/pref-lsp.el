...
(defvar pref/persist-lsp-disable-dirs nil
...
(with-eval-after-load 'savehist
  (when pref/persist-lsp-disable-dirs
    (add-to-list 'savehist-additional-variables 'pref/lsp-disable-dirs)))

(defun pref.lsp/suppress-p (&rest _args)
...
Used as a :before-until advice for `lsp' and `lsp-deferred'."
  (when-let* ((name (buffer-file-name)))
    (let ((suppress (cl-some (lambda (dir) (string-prefix-p dir name))
      ...
      (when suppress
        (message "[PREF] Disabled lsp by pref/lsp-disable-dirs rule."))
      suppress)))

(advice-add 'lsp :before-until #'pref.lsp/suppress-p)
(advice-add 'lsp-deferred :before-until #'pref.lsp/suppress-p)
